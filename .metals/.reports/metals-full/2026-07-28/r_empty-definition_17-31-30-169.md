error id: file://<WORKSPACE>/src/main/java/com/microservices/gateway/filter/AuthenticationFilter.java:_empty_/JwtUtil#extractUserId#
file://<WORKSPACE>/src/main/java/com/microservices/gateway/filter/AuthenticationFilter.java
empty definition using pc, found symbol in pc: _empty_/JwtUtil#extractUserId#
...
public class AuthenticationFilter implements GlobalFilter, Ordered {
	...
	private static final List<String> PUBLIC_ENDPOINTS = List.of(
			...
			"/api/v1/s3/health", "/api/v1/ec2/health", "/api/v1/rds/health", "/api/v1/lambda/health",
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		...
		case "jwt":
		case "query":
			System.out.println("====> uwthMethod: " + authMethod + " Url: " + credential);

			if (!jwtUtil.isTokenValid(credential)) {
				return failAuth(exchange, path, "JWT_SIGNATURE");
			}

			userId = jwtUtil.extractU@@serId(credential);
